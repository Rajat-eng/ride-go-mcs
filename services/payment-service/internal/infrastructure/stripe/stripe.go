...
// not *domain.PaymentProcessor bcoz interfaces are already refernce like types
// if it was truct then we ciuld return pointer wiyth *

// stripeCliet implements createSession method which implies that NewStripeClient must implement createSession methids
// var x paymentneProcessor = NewStripeClient(config)

// we return interface instaead of stripeClient bcoz later if we add another payment service then no problen since it dependds on interface not sstripeClient
